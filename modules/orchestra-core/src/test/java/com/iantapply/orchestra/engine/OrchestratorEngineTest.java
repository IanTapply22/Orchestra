...
class OrchestratorEngineTest {
    ...
    void advancesAllStagesAndDoesNotRepeatActions() throws Exception {
        ...
        try (OrchestratorEngine engine = new OrchestratorEngine(
            ...
            engine.start();
            awaitStatus(() -> stores.find(id).orElseThrow().status() == EventStatus.COMPLETED);
    ...
    void failsExecutionWhenAConditionDoesNotPass() throws Exception {
        ...
        try (OrchestratorEngine engine = new OrchestratorEngine(
            ...
            awaitStatus(() -> stores.find(id).orElseThrow().status() == EventStatus.FAILED);
    ...
    void recoversStartingExecutionsAndRunsTheirAction() throws Exception {
        ...
        try (OrchestratorEngine engine = new OrchestratorEngine(
            ...
            engine.recover();
            engine.start();
            awaitStatus(() -> stores.find(starting.id()).orElseThrow().status() == EventStatus.COMPLETED);
            assertEquals(1, calls.get());
        }
    }
    ...
    private static void awaitStatus(java.util.function.BooleanSupplier condition) {
        await().atMost(Duration.ofSeconds(3)).until(() -> condition.getAsBoolean());
    }
}
